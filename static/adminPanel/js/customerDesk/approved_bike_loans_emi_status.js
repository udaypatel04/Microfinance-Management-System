...
async function fetchInstallments() {
    ...
    try {
        ...
        // Reset global array with database records
       ...
       if (installments.length < totalTenure) {
            ...
            for (let i = 1; i <= remainingCount; i++) {
                ...
                // 3. Build the YYYY-MM-DD string MANUALLY (Avoiding UTC shift)
        ...
        if (container) {
            container.innerHTML = `
                <div class="col-span-full py-20 text-center animate__animated animate__fadeIn">
                    <p class="text-rose-500 font-black uppercase tracking-widest text-xs">Failed to load repayment schedule</p>
                    <p class="text-slate-400 text-[10px] mt-1">Please check server logs for Request ID: ${requestID}</p>
                </div>`;
        }
    }
}
...
function renderSchedule() {
    ...
    // Set container to grid for square tiles
    ...
    paginatedData.forEach((item, index) => {
        ...
        const statusConfigs = {
            'paid': { 
                ...
                btn: `
                    <button onclick="printReceipt(${item.no},${loanRequestID})" class="w-full mt-2 py-2 rounded-xl bg-white border border-slate-100 text-slate-400 flex items-center justify-center hover:bg-blue-50 hover:text-blue-600 transition-all shadow-sm">
            ...
            'pending': { 
                style: 'border-t-blue-500 bg-white shadow-blue-900/5', 
        ...
        container.innerHTML += `
            <div class="p-6 rounded-3xl border border-slate-100 border-t-4 ${config.style} shadow-sm flex flex-col items-center text-center group hover:shadow-xl hover:-translate-y-1 transition-all duration-300 animate__animated animate__fadeInUp" style="animation-delay: ${index * 0.1}s">
                ...
                <div class="w-12 h-12 mb-4 rounded-2xl bg-white border border-slate-100 text-slate-400 group-hover:border-blue-200 group-hover:text-blue-600 flex items-center justify-center font-black text-lg transition-all shadow-sm shrink-0">
                    ${item.no.toString().padStart(2, '0')}
                </div>

                <div class="mb-4">
                    <span class="text-[10px] text-slate-400 font-black uppercase block mb-1"><p>Due Date</p></span>
                ...
                <div class="mb-4">
                    ...
                    <p class="text-[11px] text-blue-500 font-bold">${item.emiTime || '--:--'}</p>
                </div>
...
function updatePaginationUI() {
    ...
    if (document.getElementById('endRange')) {
